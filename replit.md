...
The backend is built with Node.js, TypeScript, Express.js, and Drizzle ORM (PostgreSQL). It provides RESTful APIs for managing core entities, file uploads, sample data, and configuration. Data is stored in a PostgreSQL database (Neon serverless). **Local File Fallback (November 24, 2025)**: Azure Blob Storage service implements automatic fallback to local filesystem (/tmp/intellibid-documents/) when Azure is unavailable, with automatic retry every 60 seconds. Features defense-in-depth security with path traversal prevention at both route (/api/files/) and service levels (sanitizeBlobName), whitelisted prefix enforcement (knowledge-base/, rft-generation/, proposals/, evaluations/, templates/), and storage status API (getStorageStatus) for backend observability. Enables production deployment without Azure Storage dependency while maintaining automatic migration to Azure when connectivity is restored.
...
- **Knowledge Base & RAG Infrastructure:** Utilizes Azure Embedding Service, Intelligent Chunking, Azure Blob Storage (with local filesystem fallback), and Azure AI Search for document ingestion, processing, and retrieval, organized by AI agent role for targeted knowledge. **OCR Toggle Feature (November 24, 2025)**: Admin-configurable OCR enable/disable setting provides dual-mode operation: (1) OCR Enabled - Uses Azure AI Search OCR skillset for extracting text from images and scanned documents (requires AZURE_COGNITIVE_SERVICES_KEY), (2) OCR Disabled - Direct text embedding approach bypasses skillset infrastructure, enabling production deployment without Cognitive Services configuration. Setting stored in systemConfig table, checked during skillset initialization and document ingestion. Perfect fallback for immediate deployment when OCR infrastructure is not yet configured.
- **Data Management:** Includes a mock data generation page for creating persistent RFT scenarios and evaluation reports, with documents stored in Azure Blob Storage or local filesystem fallback.
- **File Storage Resilience:** All document operations (upload, download, delete, list) automatically try Azure Blob Storage first and fall back to local filesystem if Azure is unavailable. Automatic retry logic attempts Azure reconnection every 60 seconds, enabling seamless migration back to Azure when connectivity is restored. Multi-layered security prevents path traversal attacks through input validation, blob name sanitization, and path resolution verification.
