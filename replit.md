...
- **Agent Metrics Standalone Page (November 11, 2025)**: Agent Metrics dashboard moved from Admin Config tab to dedicated menu item for improved visibility and user experience. Created standalone AgentMetricsPage.tsx accessible via /agent-metrics route with comprehensive metrics dashboard featuring summary stats, per-agent performance, and recent failures. Admin Config simplified to 5 tabs (Agents, AI Search, Storage, Azure OpenAI, RAG) with Metrics removed. Navigation sidebar includes "Agent Metrics" menu item positioned after Admin Config using Activity icon. All functionality preserved with proper page header, layout consistency with other pages, and full dark mode support.
- **Agent Metrics & Observability (November 11, 2025)**: Comprehensive metrics tracking system for the 6-agent evaluation pipeline with real-time analytics. Features include: (1) PostgreSQL-backed AgentMetricsService tracking execution time, token usage, cost estimation (GPT-4o pricing), success rates, and error classification for each agent execution; (2) RESTful API endpoints (/api/agent-metrics/*) exposing summary stats, per-agent performance, recent failures, and time-series data; (3) Enterprise dashboard displaying total executions, success rates, avg execution time, total cost, per-agent breakdowns with color-coded success indicators, and detailed failure logs with error types and timestamps; (4) Automatic cost estimation assuming 70% input / 30% output token ratio; (5) Structured JSON logging for external monitoring systems. All metrics persisted to database with 7-day retention via query-time filtering, indexed for high-performance analytics queries, and foreign-key constrained to evaluations/projects tables for referential integrity.
- **Externalized Agent Prompts**: Multi-agent evaluation prompts refactored from inline TypeScript strings to separate Markdown files in `server/prompts/` directory. Each of the 6 agents (Delivery, Product, Architecture, Engineering, Procurement, Security) now has a dedicated MD file with standardized structure (## System Prompt, ## User Template). This enables non-developers to easily update agent instructions, improves version control for prompt changes, and significantly reduces code file size. The `loadAgentPrompt()` function parses these files at startup using regex pattern matching.
- **Context-Aware Questionnaire Generation**: Dramatically enhanced AI prompt engineering for all 4 questionnaire types (Product, NFR, Cybersecurity, Agile) to deeply integrate business case details. Each questionnaire now generates questions that directly reference specific requirements, risks, success criteria, stakeholders, timeline, and budget from the user's lean business case. Product questionnaires dynamically allocate questions per requirement (~2-3 each), NFR questions address identified risks, Cybersecurity questions reference stakeholder groups, and Agile questions align with project timelines. All prompts include GOOD vs BAD examples and CRITICAL INSTRUCTIONS forbidding generic template questions. This ensures Smart RFT Builder generates 30 Product, 50 NFR, 20 Cybersecurity, and 20 Agile questions that are tailored to the exact project context.
